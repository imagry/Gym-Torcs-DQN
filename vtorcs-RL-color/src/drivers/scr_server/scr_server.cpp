...
//    std::cerr << "totraced: " << totdist << std::endl;
    ...
    carState.set_angle(angle);
    carState.set_curlaptime(float(car->_curLapTime));
    carState.set_damage(( getDamageLimit() ? car->_dammage : car->_fakeDammage ) );
    carState.set_distfromstart(car->race.distFromStartLine);
    carState.set_totaldistfromstart(totdist);
    carState.set_distraced(distRaced[index]);
    carState.set_fuel(car->_fuel);
    carState.set_gear(car->_gear);
    carState.set_lastlaptime(float(car->_lastLapTime));
    for (int i = 0; i < 36; ++i) {
        carState.add_opponents(oppSensorOut[i]);
    }
    //carState.set_racepos(car->race.pos);
    carState.set_rpm(car->_enginerpm*10);
    carState.set_speedx(float(car->_speed_x * 3.6));
    carState.set_speedy(float(car->_speed_y * 3.6));
    carState.set_speedz(float(car->_speed_z * 3.6));
    /*for (int i = 0; i < 19; ++i) {
    ...
    carState.set_z(car->_pos_Z  - RtTrackHeightL(&(car->_trkPos)));
    for (int i = 0; i < 5; ++i) {
        carState.add_focus(focusSensorOut[i]);
    }
    carState.set_steering(car->_steerCmd);
    carState.set_acceleration(car->_accelCmd);
    carState.set_breaking(car->_brakeCmd);
    ...
    // Image encoding to jpeg:
    {
        ...
        if (cv::imencode(".jpg", image, encoded_image, ENCODING_PARAMETERS)) {
            carState.set_img(&encoded_image[0], encoded_image.size());
        } else {
            std::cerr << "Can't encode image to jpeg" << std::endl;
        }
    }

    carState.SerializeToString(&stateString);
...
//    if (sendto(listenSocket[index], line, strlen(line) + 1, 0,
    ...
    //std::cout << "Car state size: " << defstream.total_out << std::endl;
