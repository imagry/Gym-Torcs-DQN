	...
	if ((s->currentTime - ReInfo->_reLastTime) >= RCM_MAX_DT_ROBOTS) {
		...
		for (i = 0; i < s->_ncars; i++) {
			if ((s->cars[i]->_state & RM_CAR_STATE_NO_SIMU) == 0) {
				...
				if (i == 1 && getObserver()) {
    				// Dirty hack to send to a second car (capturing driver) state of a first car (AI)
				    robot->rbDrive(robot->index, s->cars[0], s);
				    // Restart original robot
				    // s->cars[0]->robot->rbDrive(robot->index, s->cars[0], s);
				} else {
