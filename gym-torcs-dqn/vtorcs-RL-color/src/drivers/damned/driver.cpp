...
const float Driver::WIDTHDIV = 10.0f;						// [-] Defines the percentage of the track to use (2/WIDTHDIV).
const float Driver::SIDECOLL_MARGIN = 3.0f;					// [m] Distance between car centers to avoid side collisions.
const float Driver::BORDER_OVERTAKE_MARGIN = 3.0f;			// [m]
...
const float Driver::MAX_SPEED = 30.0f;						// [m/s] Speed to compute the percentage of brake to apply.
...
const float Driver::INTERVENTION_INTENSITY = 0.005f;         // Periodically steer out of the center
const float Driver::TEAM_REAR_DIST = 50.0f;					//
const int Driver::INTERVENTION_PERIOD = 200;
...
void Driver::drive(tSituation *s)
{
	...
	if (isStuck()) {
		...
		if (INTERVENTION_PERIOD != 0) {
            int iteration = int(currentsimtime * 10);
            if (iteration % INTERVENTION_PERIOD < 10) {
                int sign = (iteration / INTERVENTION_PERIOD) % 2 ? -1 : 1;
                int intervention_coefficient = 1.0 + (iteration / INTERVENTION_PERIOD) % 11;
                car->_steerCmd = INTERVENTION_INTENSITY * sign * intervention_coefficient;
            }
        }
